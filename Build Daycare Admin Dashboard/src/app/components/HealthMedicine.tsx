    ...
    <div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                ...
                <div className="flex gap-2 items-center">
                  <input type="number" step="0.1" value={tempEdit} onChange={e => setTempEdit(e.target.value)}
      ...
      {(modal === "addRecord" || modal === "editRecord") && (
        <Modal title={modal === "addRecord" ? "Add Medical Record" : "Edit Record"} onClose={() => setModal(null)}>
          <div className="grid grid-cols-2 gap-4">
            <Input label="Child Name" value={form.childName} onChange={v => setForm(p => ({ ...p, childName: v }))} />
            <Select label="Type" value={form.type} onChange={v => setForm(p => ({ ...p, type: v as HealthRecord["type"] }))} options={[...HEALTH_TYPES]} />
            <Input label="Date" type="date" value={form.date} onChange={v => setForm(p => ({ ...p, date: v }))} />
            <Input label="Nurse / Staff" value={form.nurse} onChange={v => setForm(p => ({ ...p, nurse: v }))} />
            <Select label="Status" value={form.status} onChange={v => setForm(p => ({ ...p, status: v }))} options={["Active", "Resolved"]} />
            <div className="col-span-2"><Textarea label="Description" value={form.description} onChange={v => setForm(p => ({ ...p, description: v }))} /></div>
          </div>
      ...
      {modal === "addMed" && selected && (
        <Modal title={`Add Medicine — ${selected.childName}`} onClose={() => setModal("view")}>
          <div className="space-y-3">
            ...
            <Select label="Frequency" value={medForm.frequency} onChange={v => setMedForm(p => ({ ...p, frequency: v }))} options={["Once daily", "Twice daily", "Three times daily", "As needed", "Weekly"]} />
            <div className="grid grid-cols-2 gap-3">
              <Input label="Start Date" type="date" value={medForm.startDate} onChange={v => setMedForm(p => ({ ...p, startDate: v }))} />
              <Input label="End Date" type="date" value={medForm.endDate} onChange={v => setMedForm(p => ({ ...p, endDate: v }))} />
            </div>
      ...
      {modal === "growth" && selected && (
        <Modal title={`Add Growth Record — ${selected.childName}`} onClose={() => setModal("view")}>
          <div className="grid grid-cols-2 gap-4">
            <Input label="Weight (kg)" type="number" step="0.1" value={growthForm.weight} onChange={v => setGrowthForm(p => ({ ...p, weight: v }))} placeholder="e.g. 13.5" />
            <Input label="Height (cm)" type="number" step="0.5" value={growthForm.height} onChange={v => setGrowthForm(p => ({ ...p, height: v }))} placeholder="e.g. 92" />
            <div className="col-span-2"><Textarea label="Notes" value={growthForm.notes} onChange={v => setGrowthForm(p => ({ ...p, notes: v }))} rows={2} /></div>
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Btn variant="secondary" onClick={() => setModal("view")}>Cancel</Btn>
