  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={f.key} className="flex flex-col gap-1">
                ...
                <select value={filters[f.key]} onChange={e => filt(f.key, e.target.value)}
      ...
      <Modal open={modal === "pickup-add"} onClose={() => setModal("profile")} title="Add Authorized Pickup Person">
        <div className="space-y-3">
          <div className="flex flex-col gap-1"><label className="text-sm text-gray-600">Full Name</label><input value={newPickup.name} onChange={e => setNewPickup(p => ({ ...p, name: e.target.value }))} className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
          <div className="flex flex-col gap-1"><label className="text-sm text-gray-600">Relationship</label>
            <select value={newPickup.relationship} onChange={e => setNewPickup(p => ({ ...p, relationship: e.target.value as PickupPerson["relationship"] }))}
          ...
          <div className="flex flex-col gap-1"><label className="text-sm text-gray-600">Phone</label><input value={newPickup.phone} onChange={e => setNewPickup(p => ({ ...p, phone: e.target.value }))} className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
          <div className="flex items-center gap-2"><input type="checkbox" id="phid" checked={newPickup.photoId} onChange={e => setNewPickup(p => ({ ...p, photoId: e.target.checked }))} className="w-4 h-4" /><label htmlFor="phid" className="text-sm text-gray-600">Photo ID verified</label></div>
        </div>
        <div className="flex justify-end gap-2 mt-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[["Full Name", "name"], ["Email", "email"], ["Phone", "phone"], ["Alt Phone", "alternatePhone"], ["Occupation", "occupation"], ["Address", "address"], ["Emergency Contact", "emergencyContact"]].map(([label, key]) => (
            <div key={key} className={`flex flex-col gap-1 ${key === "address" || key === "emergencyContact" ? "col-span-2" : ""}`}>
              ...
              <input value={(form as any)[key] ?? (selected as any)?.[key] ?? ""} onChange={e => ff(key, e.target.value)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          ))}
          <div className="flex flex-col gap-1">
            <label className="text-sm text-gray-600">Status</label>
            <select value={(form as any).status ?? "Active"} onChange={e => ff("status", e.target.value)}
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500">
