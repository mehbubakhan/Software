...
const PAYMENT_METHOD_ICONS: Record<PaymentMethod, JSX.Element> = {
...
const emptyForm = {
  ...
  status: "Pending" as Invoice["status"], description: "", packageType: "1 Month" as PackageType,
      ...
      {(modal === "add" || modal === "edit") && (
        <Modal title={modal === "add" ? "Create Invoice" : "Edit Invoice"} onClose={() => setModal(null)} size="lg">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Parent Name" value={form.parentName} onChange={v => f("parentName", v)} />
            <Input label="Child Name" value={form.childName} onChange={v => f("childName", v)} />
            <Select label="Invoice Type" value={form.invoiceType} onChange={v => f("invoiceType", v)} options={["Monthly", "Admission", "Transport", "Miscellaneous"]} />
            <Select label="Package" value={form.packageType} onChange={v => { f("packageType", v); f("amount", PACKAGE_PRICES[v as PackageType] ?? 750); }} options={Object.keys(PACKAGE_PRICES)} />
            <Input label="Amount ($)" type="number" value={String(form.amount)} onChange={v => f("amount", Number(v))} />
            <Input label="Discount ($)" type="number" value={String(form.discountAmount)} onChange={v => f("discountAmount", Number(v))} />
            <Input label="Issue Date" type="date" value={form.issueDate} onChange={v => f("issueDate", v)} />
            <Input label="Due Date" type="date" value={form.dueDate} onChange={v => f("dueDate", v)} />
            <Select label="Status" value={form.status} onChange={v => f("status", v)} options={["Pending", "Paid", "Overdue", "Cancelled"]} />
            <div className="col-span-2"><Input label="Description / Notes" value={form.description} onChange={v => f("description", v)} /></div>
          </div>
