...
export function Input({ label, onChange, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label?: string; onChange?: ((v: string) => void) | React.ChangeEventHandler<HTMLInputElement> }) {
  return (
...
export function Select({ label, children, options, onChange, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string; options?: string[]; onChange?: (v: string) => void }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-sm text-gray-600">{label}</label>}
      <select
        ...
        onChange={onChange ? e => onChange(e.target.value) : props.onChange as React.ChangeEventHandler<HTMLSelectElement>}
        {...props}
      >
...
export function Textarea({ label, onChange, ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label?: string; onChange?: ((v: string) => void) | React.ChangeEventHandler<HTMLTextAreaElement> }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-sm text-gray-600">{label}</label>}
      <textarea
