...
interface AdmissionExt extends Admission {
  admissionId: string;
  // Parent extra
  parentPhone: string;
    ...
    <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {([
              ...
              <div key={f.key} className="flex flex-col gap-1">
                ...
                <select value={(filters as any)[f.key]} onChange={e => filt(f.key as any, e.target.value)}
      ...
      <Modal open={modal === "add" || modal === "edit"} onClose={() => setModal(null)}
        ...
        <div className="space-y-5">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <F label="Parent Full Name"><input value={form.parentName} onChange={e => ff("parentName", e.target.value)} className={INP} /></F>
              <F label="Parent Email"><input type="email" value={form.parentEmail} onChange={e => ff("parentEmail", e.target.value)} className={INP} /></F>
              <F label="Phone"><input value={form.parentPhone} onChange={e => ff("parentPhone", e.target.value)} className={INP} /></F>
              <F label="Occupation"><input value={form.parentOccupation} onChange={e => ff("parentOccupation", e.target.value)} className={INP} /></F>
              <div className="col-span-2"><F label="Address"><input value={form.parentAddress} onChange={e => ff("parentAddress", e.target.value)} className={INP} /></F></div>
              <F label="Relationship"><select value={form.parentRelationship} onChange={e => ff("parentRelationship", e.target.value)} className={SEL}>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <F label="Child Name"><input value={form.childName} onChange={e => ff("childName", e.target.value)} className={INP} /></F>
              <F label="Date of Birth"><input type="date" value={form.dob} onChange={e => ff("dob", e.target.value)} className={INP} /></F>
              <F label="Age"><input type="number" value={form.childAge} onChange={e => ff("childAge", Number(e.target.value))} className={INP} min={0} max={10} /></F>
              <F label="Allergies"><input value={form.childAllergies} onChange={e => ff("childAllergies", e.target.value)} placeholder="None / Peanuts…" className={INP} /></F>
              <F label="Vaccination"><select value={form.childVaccination} onChange={e => ff("childVaccination", e.target.value)} className={SEL}>
                <option>Up to date</option><option>Partial</option><option>Not vaccinated</option>
              </select></F>
              <F label="Special Needs"><div className="flex items-center gap-2 h-10"><input type="checkbox" checked={form.childSpecialNeeds} onChange={e => ff("childSpecialNeeds", e.target.checked)} className="w-4 h-4" /><span className="text-sm text-gray-600">Has special needs</span></div></F>
              <div className="col-span-2"><F label="Medical History"><textarea rows={2} value={form.childMedicalHistory} onChange={e => ff("childMedicalHistory", e.target.value)} className={`${INP} resize-none`} /></F></div>
            </div>
          </section>
          {/* Package */}
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <F label="Start Date"><input type="date" value={form.packageStart} onChange={e => { ff("packageStart", e.target.value); ff("packageEnd", calcEnd(e.target.value, form.package)); }} className={INP} /></F>
              <F label="End Date (auto)"><input value={form.packageEnd} readOnly className={`${INP} bg-gray-100 cursor-not-allowed`} /></F>
              <F label="Status"><select value={form.status} onChange={e => ff("status", e.target.value)} className={SEL}>
                {["Pending", "Approved", "Waitlisted", "Rejected", "Suspended", "Info Requested"].map(s => <option key={s}>{s}</option>)}
              </select></F>
              <F label="Request Date"><input type="date" value={form.requestDate} onChange={e => ff("requestDate", e.target.value)} className={INP} /></F>
              <div className="col-span-2"><F label="Notes"><textarea rows={2} value={form.notes} onChange={e => ff("notes", e.target.value)} className={`${INP} resize-none`} /></F></div>
            </div>
          </section>
        </div>
      ...
      <Modal open={modal === "visit"} onClose={() => setModal(null)} title={`Schedule Visit — ${selected?.childName}`}>
        <div className="space-y-3">
          ...
          <F label="Visit Date"><input type="date" value={visitForm.date} onChange={e => setVisitForm(p => ({ ...p, date: e.target.value }))} className={INP} /></F>
          <F label="Visit Time"><input value={visitForm.time} onChange={e => setVisitForm(p => ({ ...p, time: e.target.value }))} placeholder="e.g. 10:00 AM" className={INP} /></F>
          <F label="Note for Parent"><textarea rows={3} value={visitForm.note} onChange={e => setVisitForm(p => ({ ...p, note: e.target.value }))} placeholder="Any instructions or details…" className={`${INP} resize-none`} /></F>
        </div>
      ...
      <Modal open={modal === "moreinfo"} onClose={() => setModal(null)} title={`Request Info — ${selected?.childName}`}>
        <div className="space-y-3">
          ...
          <textarea rows={5} value={moreInfoNote} onChange={e => setMoreInfoNote(e.target.value)}
      ...
      <Modal open={modal === "trial"} onClose={() => setModal(null)} title="Book Trial Admission / Daycare Visit" width="max-w-lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <F label="Parent Name"><input value={trialForm.parentName} onChange={e => setTrialForm(p => ({ ...p, parentName: e.target.value }))} className={INP} /></F>
            <F label="Parent Phone"><input value={trialForm.parentPhone} onChange={e => setTrialForm(p => ({ ...p, parentPhone: e.target.value }))} className={INP} /></F>
            <F label="Child Name"><input value={trialForm.childName} onChange={e => setTrialForm(p => ({ ...p, childName: e.target.value }))} className={INP} /></F>
            <F label="Child Age"><input type="number" value={trialForm.childAge} onChange={e => setTrialForm(p => ({ ...p, childAge: e.target.value }))} className={INP} min={0} max={10} /></F>
            <F label="Preferred Visit/Start Date"><input type="date" value={trialForm.visitDate} onChange={e => setTrialForm(p => ({ ...p, visitDate: e.target.value }))} className={INP} /></F>
          </div>
          <F label="Notes / Special Requests"><textarea rows={3} value={trialForm.note} onChange={e => setTrialForm(p => ({ ...p, note: e.target.value }))} placeholder="Any special requirements…" className={`${INP} resize-none w-full`} /></F>
        </div>
        <div className="flex justify-end gap-2 mt-5">
          <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
