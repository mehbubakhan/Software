    ...
    <div>
      ...
      {(modal === "addActivity" || modal === "editActivity") && (
        <Modal title={modal === "addActivity" ? "Add Activity" : "Edit Activity"} onClose={() => setModal(null)} size="lg">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><Input label="Activity Title" value={actForm.title} onChange={v => setActForm(p => ({ ...p, title: v }))} /></div>
            <Select label="Group" value={actForm.group} onChange={v => setActForm(p => ({ ...p, group: v }))} options={["All", ...GROUPS]} />
            <Select label="Type" value={actForm.type} onChange={v => setActForm(p => ({ ...p, type: v as Activity["type"] }))} options={[...ACTIVITY_TYPES]} />
            <Input label="Date" type="date" value={actForm.date} onChange={v => setActForm(p => ({ ...p, date: v }))} />
            <Input label="Time" value={actForm.time} onChange={v => setActForm(p => ({ ...p, time: v }))} placeholder="09:00 AM" />
            <div className="col-span-2"><Input label="Instructor" value={actForm.instructor} onChange={v => setActForm(p => ({ ...p, instructor: v }))} /></div>
            <Select label="Status" value={actForm.status} onChange={v => setActForm(p => ({ ...p, status: v as Activity["status"] }))} options={[...ACTIVITY_STATUSES]} />
            <div className="col-span-2"><Textarea label="Description" value={actForm.description} onChange={v => setActForm(p => ({ ...p, description: v }))} /></div>
          </div>
      ...
      {modal === "addFood" && (
        <Modal title="Log Meal" onClose={() => setModal(null)}>
          <div className="space-y-3">
            <Select label="Meal Type" value={foodForm.meal} onChange={v => setFoodForm(p => ({ ...p, meal: v as MealType }))} options={["Breakfast", "Morning Snack", "Lunch", "Afternoon Snack", "Dinner"]} />
            <Input label="Food Items" value={foodForm.items} onChange={v => setFoodForm(p => ({ ...p, items: v }))} placeholder="e.g. Rice, chicken, apple" />
            <div className="grid grid-cols-2 gap-3">
              <Select label="Amount Eaten" value={foodForm.amount} onChange={v => setFoodForm(p => ({ ...p, amount: v as FoodEntry["amount"] }))} options={["All", "Half", "Little", "None"]} />
              <Input label="Water (ml)" type="number" value={String(foodForm.waterMl)} onChange={v => setFoodForm(p => ({ ...p, waterMl: Number(v) }))} />
            </div>
            <Input label="Time" type="time" value={foodForm.time} onChange={v => setFoodForm(p => ({ ...p, time: v }))} />
            <Textarea label="Notes" value={foodForm.notes} onChange={v => setFoodForm(p => ({ ...p, notes: v }))} rows={2} />
          </div>
      ...
      {modal === "addSleep" && (
        <Modal title="Log Sleep" onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Input label="Sleep Time" type="time" value={sleepForm.sleepTime} onChange={v => setSleepForm(p => ({ ...p, sleepTime: v }))} />
              <Input label="Wake Time" type="time" value={sleepForm.wakeTime} onChange={v => setSleepForm(p => ({ ...p, wakeTime: v }))} />
            </div>
            <Select label="Sleep Quality" value={sleepForm.quality} onChange={v => setSleepForm(p => ({ ...p, quality: v as SleepQuality }))} options={["Good", "Fair", "Poor", "Restless"]} />
            <Textarea label="Notes" value={sleepForm.notes} onChange={v => setSleepForm(p => ({ ...p, notes: v }))} rows={2} />
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
      ...
      {modal === "addEducation" && (
        <Modal title="Add Education Activity" onClose={() => setModal(null)}>
          <div className="space-y-3">
            <Select label="Category" value={eduForm.category} onChange={v => setEduForm(p => ({ ...p, category: v as EducationEntry["category"] }))} options={["Alphabet", "Numbers", "Drawing", "Reading", "Story", "Craft", "Music", "Science"]} />
            <Input label="Activity Description" value={eduForm.activity} onChange={v => setEduForm(p => ({ ...p, activity: v }))} placeholder="e.g. Alphabet A–D practice" />
            <Textarea label="Notes" value={eduForm.notes} onChange={v => setEduForm(p => ({ ...p, notes: v }))} rows={2} />
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
      ...
      {modal === "addMood" && (
        <Modal title="Log Mood" onClose={() => setModal(null)}>
          <div className="space-y-3">
            ...
            <Textarea label="Notes" value={moodForm.notes} onChange={v => setMoodForm(p => ({ ...p, notes: v }))} rows={2} placeholder="What caused this mood?" />
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
      ...
      {modal === "addMedicine" && (
        <Modal title="Add Medicine" onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Input label="Medicine Name" value={medForm.medicine} onChange={v => setMedForm(p => ({ ...p, medicine: v }))} />
              <Input label="Dosage" value={medForm.dosage} onChange={v => setMedForm(p => ({ ...p, dosage: v }))} placeholder="e.g. 5ml" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Input label="Schedule Time" type="time" value={medForm.time} onChange={v => setMedForm(p => ({ ...p, time: v }))} />
              <Input label="Given By" value={medForm.givenBy} onChange={v => setMedForm(p => ({ ...p, givenBy: v }))} />
            </div>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={medForm.parentApproved} onChange={e => setMedForm(p => ({ ...p, parentApproved: e.target.checked }))} className="rounded" />
              <span className="text-sm text-gray-600">Parent has approved this medicine</span>
            </label>
            <Textarea label="Notes" value={medForm.notes} onChange={v => setMedForm(p => ({ ...p, notes: v }))} rows={2} />
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
