...
export default function AdvancedFeatures() {
  ...
  const [behaviourData, setBehaviourData] = useState<BehaviourAnalysis[]>([]);
  ...
  const analyzeBehaviour = (childId: string) => {
  ...
  const checkPermissions = (role: string) => {
    const permissions = {
    ...
    toast.success(`${role} permissions:`, {
      description: permissions[role as keyof typeof permissions]?.join(', ')
    });
  };
  ...
  const toggleAutomation = (feature: string) => {
    ...
    toast.success(`${feature} automation ${!automationEnabled[feature as keyof typeof automationEnabled] ? 'enabled' : 'disabled'}`);
  };
                        ...
                        <div className="relative w-20 h-20 mx-auto">
                          <svg className="w-full h-full transform -rotate-90">
                            ...
                            <circle
                              ...
                              strokeWidth="6"
                              strokeDasharray={`${(Number(value) / 100) * 220} 220`}
                            />
                          </svg>
