...
export default function DaycarePayment() {
  ...
  return (
    <div className="bg-[#111322] min-h-[calc(100vh-68px)] text-slate-100 -m-6 p-8 font-sans flex flex-col items-center pb-24">
      <div className="w-full max-w-2xl mt-8">
        ...
        <div className="bg-[#1a1c2d] border border-slate-700 rounded-2xl p-8 shadow-xl">
          <div className="flex justify-between items-center mb-8 border-b border-slate-700 pb-4">
            <h1 className="text-xl font-bold text-white">Payment Method</h1>
            <div className="text-slate-400 text-sm">Fee: <span className="text-white font-bold">$1,200.00</span></div>
          </div>
          ...
          {step !== 4 && (
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button 
                ...
                className={`flex items-center justify-center gap-2 py-4 rounded-xl border transition font-bold ${paymentMethod === 'card' ? 'border-fuchsia-500 bg-fuchsia-500/10 text-white' : 'border-slate-700 text-slate-400 hover:bg-slate-800'}`}
              >
                <span>💳</span> Credit/Debit Card
              </button>
              <button 
                onClick={() => { setPaymentMethod('mobile'); setStep(1); }}
                className={`flex items-center justify-center gap-2 py-4 rounded-xl border transition font-bold ${paymentMethod === 'mobile' ? 'border-fuchsia-500 bg-fuchsia-500/10 text-white' : 'border-slate-700 text-slate-400 hover:bg-slate-800'}`}
              >
          ...
          {paymentMethod === 'card' && step === 1 && (
            <div className="space-y-4">
              <div>
                ...
                <input value={cardData.number} onChange={e => setCardData({...cardData, number: e.target.value})} type="text" placeholder="4758 9825 8957 5678" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-fuchsia-500" />
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Cardholder Name <span className="text-red-500">*</span></label>
                <input value={cardData.name} onChange={e => setCardData({...cardData, name: e.target.value})} type="text" placeholder="John Doe" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-fuchsia-500" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Expiry Date <span className="text-red-500">*</span></label>
                  <input value={cardData.expiry} onChange={e => setCardData({...cardData, expiry: e.target.value})} type="text" placeholder="MM/YY" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-fuchsia-500" />
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1">CVV <span className="text-red-500">*</span></label>
                  <input value={cardData.cvv} onChange={e => setCardData({...cardData, cvv: e.target.value})} type="password" placeholder="123" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-fuchsia-500" />
                </div>
              </div>
              ...
              <div className="grid gap-3">
                {providers.map(prov => (
                  <button 
                    ...
                    className="flex items-center justify-between p-4 rounded-xl border border-slate-700 hover:border-fuchsia-500 hover:bg-slate-800 transition"
                  >
          ...
          {paymentMethod === 'mobile' && step === 2 && (
            <div className="space-y-6">
              ...
              <div className="border border-fuchsia-500 rounded-xl p-4 flex items-center gap-4">
                ...
                <div>
                  <div className="font-bold text-white">{mobileProvider} Payment</div>
              ...
              <div>
                ...
                <input value={mobileData.number} onChange={e => setMobileData({...mobileData, number: e.target.value})} type="tel" placeholder="017... (e.g. 11 digits)" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-fuchsia-500 text-center text-lg tracking-widest" />
              </div>
          ...
          {paymentMethod === 'mobile' && step === 3 && (
            <div className="space-y-6 flex flex-col items-center text-center">
              ...
              <p className="text-slate-300">A verification code has been sent to<br/> <span className="font-bold text-white">{mobileData.number}</span></p>

              <div className="w-full max-w-sm">
                <label className="block text-sm text-slate-400 mb-1 text-left">Enter Your PIN</label>
                <input value={mobileData.pin} onChange={e => setMobileData({...mobileData, pin: e.target.value})} type="password" placeholder="••••" className="w-full bg-[#111322] border border-slate-700 rounded-xl px-4 py-4 text-white focus:outline-none focus:border-fuchsia-500 text-center text-2xl tracking-[1em]" />
              </div>
          ...
          {step === 4 && transactionData && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <div className="w-full bg-[#111322] rounded-xl p-6 space-y-4 text-sm border border-slate-700 mb-8">
                <h3 className="font-bold text-white border-b border-slate-700 pb-2 mb-4">Transaction Details</h3>
                <div className="flex justify-between">
                  <span className="text-slate-400">Amount</span>
                  <span className="font-bold text-white">{transactionData.amount}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Transaction ID</span>
                  <span className="font-mono text-white">{transactionData.id}</span>
                </div>
                <div className="flex justify-between">
