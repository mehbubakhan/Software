...
interface ComplaintExt extends Omit<Complaint, "priority" | "complaintType"> {
  complaintId: string;
...
export function Complaints() {
  ...
  useEffect(() => {
    api.get('/daycare/portal/complaints')
      .then((res: any) => setComplaints(res.data.map(enrichComplaint)))
    ...
    <div>
      ...
      {modal === "addEvidence" && selected && (
        <Modal title="Add Evidence" onClose={() => setModal("view")}>
          <div className="space-y-3">
            <Select label="Evidence Type" value={evidenceForm.type} onChange={(v: any) => setEvidenceForm(p => ({ ...p, type: v as Evidence["type"] }))} options={["Photo", "CCTV Reference", "Document", "Statement"]} />
      ...
      {(modal === "add" || modal === "edit") && (
        <Modal title={modal === "add" ? "New Complaint" : "Edit Complaint"} onClose={() => setModal(null)} size="lg">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Reporter (Parent Name)" value={form.parentName} onChange={(v: any) => setForm(p => ({ ...p, parentName: v }))} />
            <Input label="Phone" value={form.reporterPhone} onChange={(v: any) => setForm(p => ({ ...p, reporterPhone: v }))} />
            <Select label="Complaint Type" value={form.complaintType} onChange={(v: any) => setForm(p => ({ ...p, complaintType: v as ComplaintType }))} options={[...COMPLAINT_TYPES]} />
            <Select label="Priority" value={form.priority} onChange={(v: any) => setForm(p => ({ ...p, priority: v as Priority }))} options={["Low", "Medium", "High", "Critical"]} />
            <div className="col-span-2"><Input label="Subject" value={form.subject} onChange={(v: any) => setForm(p => ({ ...p, subject: v }))} /></div>
            <Input label="Child Involved" value={form.childName} onChange={(v: any) => setForm(p => ({ ...p, childName: v }))} />
            <Input label="Staff Involved" value={form.staffInvolved} onChange={(v: any) => setForm(p => ({ ...p, staffInvolved: v }))} />
            <Input label="CCTV Reference" value={form.cctvRef} onChange={(v: any) => setForm(p => ({ ...p, cctvRef: v }))} placeholder="e.g. CAM-04 @ 10:35 AM" />
            <Input label="Assigned To" value={form.assignedTo} onChange={(v: any) => setForm(p => ({ ...p, assignedTo: v }))} />
