...
export function Transportation() {
  ...
  function save() {
    if (modal === "add") {
      const newV = enrichVehicle({ ...form, id: `v${Date.now()}`, children: form.children } as any, vehicles.length);
      setVehicles(prev => [...prev, newV]);
    } else if (modal === "edit" && selected) {
      setVehicles(prev => prev.map(v => v.id === selected.id ? { ...selected, ...form, children: form.children } as any : v));
    }
    setModal(null);
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        <StatCard label="Total Vehicles" value={vehicles.length} color="fuchsia" icon={<Bus size={20} />} />
      ...
      <div className="flex gap-1 mb-5 border-b border-gray-200">
        {([["vehicles", "Vehicles & GPS"], ["schedule", "Route Schedule"], ["pickups", "Pickup & Drop"], ["drivers", "Driver Management"]] as [ViewTab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm transition-colors ${tab === t ? "text-fuchsia-600 border-b-2 border-fuchsia-600" : "text-gray-500 hover:text-gray-700"}`}>
            {label}
          </button>
        ))}
      </div>
              ...
              <tbody>
                {vehicles.map(v => (
                  <tr key={v.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-fuchsia-700">{v.vehicleId}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 flex-wrap">
                        <button onClick={() => { setSelected(v); setViewTab("gps"); setModal("view"); }} title="Track Vehicle"
                          className="p-1.5 rounded hover:bg-fuchsia-50 text-fuchsia-600 transition-colors"><Navigation size={14} /></button>
                ...
                {v.routeStops.map((stop, i) => (
                  <div key={stop.name} className="flex items-center shrink-0">
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 ${stop.completed ? "bg-fuchsia-600 border-fuchsia-600" : i === v.routeStops.findIndex(s => !s.completed) ? "bg-white border-fuchsia-400 animate-pulse" : "bg-white border-gray-300"}`}>
                      ...
                      {stop.childrenBoarded > 0 && <p className="text-xs text-fuchsia-500">+{stop.childrenBoarded}</p>}
                    </div>
                    {i < v.routeStops.length - 1 && (
                      <div className={`h-0.5 w-12 mb-6 ${stop.completed ? "bg-fuchsia-400" : "bg-gray-200"}`} />
                    )}
                  </div>
                ))}
              </div>
          ...
          {vehicles.map(v => (
            <Card key={v.id} className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <Bus size={16} className="text-fuchsia-500" />
                <h3 className="text-sm" style={{ fontWeight: 600 }}>{v.name} — {v.route}</h3>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {record.status === "Pending" && (
                        <button onClick={() => confirmPickup(v.id, record.id)}
                          className="px-2 py-0.5 rounded-lg text-xs bg-fuchsia-50 text-fuchsia-600 hover:bg-fuchsia-100 transition-colors">
                          Confirm
                        </button>
                      )}
                    </div>
              ...
              <div className="col-span-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {selected.assignedChildren.map(name => (
                    <span key={name} className="px-2 py-0.5 rounded-full text-xs bg-fuchsia-50 text-fuchsia-700 border border-fuchsia-200">{name}</span>
                  ))}
                </div>
              </div>
            </div>
              ...
              <div className="relative bg-gradient-to-br from-green-50 to-blue-50 rounded-xl overflow-hidden mb-4" style={{ height: 240 }}>
                ...
                <div className="absolute transition-all duration-1000" 
                  ...
                  <div className="relative">
                    <div className="w-8 h-8 rounded-full bg-fuchsia-600 flex items-center justify-center shadow-lg transform transition-transform scale-110">
                ...
                <div className="absolute bottom-2 left-2 flex flex-col gap-1 items-start">
                  <div className="bg-white/90 rounded-lg px-2 py-1 text-xs text-gray-700 shadow-sm border border-gray-200">
                    <Navigation size={10} className="inline mr-1 text-fuchsia-500 animate-spin" style={{ animationDuration: "3s" }} />
                ...
                <div className="absolute bottom-2 right-2 bg-white/80 rounded-lg px-2 py-1 text-xs text-fuchsia-600 shadow-sm" style={{ fontWeight: 600 }}>
                  ETA {selected.eta}
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {record.status === "Pending" && (
                      <button onClick={() => confirmPickup(selected.id, record.id)}
                        className="px-2 py-0.5 rounded text-xs bg-fuchsia-50 text-fuchsia-600 hover:bg-fuchsia-100">Confirm</button>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              <div className="flex items-center overflow-x-auto pb-3 gap-0">
                {selected.routeStops.map((stop, i) => (
                  <div key={stop.name} className="flex items-center shrink-0">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${stop.completed ? "bg-fuchsia-600 border-fuchsia-600" : i === selected.routeStops.findIndex(s => !s.completed) ? "bg-white border-fuchsia-400" : "bg-white border-gray-300"}`}>
                        {stop.completed ? <CheckCircle size={14} className="text-white" /> : <span className="text-xs text-gray-400">{i + 1}</span>}
                      </div>
                    ...
                    {i < selected.routeStops.length - 1 && (
                      <div className={`h-0.5 w-10 mb-6 ${stop.completed ? "bg-fuchsia-400" : "bg-gray-200"}`} />
                    )}
                  </div>
                ))}
              </div>
      ...
      {(modal === "add" || modal === "edit") && (
        <Modal title={modal === "add" ? "Add Vehicle" : "Edit Vehicle"} onClose={() => setModal(null)} size="lg">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Vehicle Name" value={form.name} onChange={(v: any) => f("name", v)} />
            <Input label="License Plate" value={form.plate} onChange={(v: any) => f("plate", v)} />
            <Select label="Vehicle Type" value={form.vehicleType} onChange={(v: any) => f("vehicleType", v)} options={["Bus", "Van", "Car"]} />
            <Input label="Year" value={form.year} onChange={(v: any) => f("year", v)} />
            <Input label="Color" value={form.color} onChange={(v: any) => f("color", v)} />
            <Input label="Capacity" type="number" value={String(form.capacity)} onChange={(v: any) => f("capacity", Number(v))} />
            <Input label="Driver Name" value={form.driver} onChange={(v: any) => f("driver", v)} />
            <Input label="Driver Phone" value={form.driverPhone} onChange={(v: any) => f("driverPhone", v)} />
            <Input label="Driver License" value={form.driverLicense} onChange={(v: any) => f("driverLicense", v)} />
            <Select label="Route" value={form.route} onChange={(v: any) => f("route", v)} options={ROUTES} />
            <Input label="Insurance No." value={form.insurance} onChange={(v: any) => f("insurance", v)} />
            <Select label="Status" value={form.status} onChange={(v: any) => f("status", v)} options={["Active", "En Route", "Inactive"]} />
          </div>
            ...
            <div className="mt-3 flex flex-wrap gap-1">
              {CHILD_NAMES.filter(n => !selected.assignedChildren.includes(n)).slice(0, 6).map(n => (
                <button key={n} onClick={() => setAssignChild(n)}
                  className="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-600 hover:bg-fuchsia-50 hover:text-fuchsia-600 border border-gray-200">{n}</button>
              ))}
            </div>
          </div>
