...
const emptyForm = {
  ...
  shift: "Morning", group: "Sunflower", status: "Active" as StaffExt["status"], joinDate: "2026-06-04",
      ...
      {(modal === "add" || modal === "edit") && (
        <Modal title={modal === "add" ? "Add Staff Member" : "Edit Staff Member"} onClose={() => setModal(null)} size="lg">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Full Name" value={form.name} onChange={(v: any) => f("name", v)} />
            <Select label="Role" value={form.role} onChange={(v: any) => f("role", v)} options={ROLES} />
            <Input label="Email" value={form.email} onChange={(v: any) => f("email", v)} />
            <Input label="Phone" value={form.phone} onChange={(v: any) => f("phone", v)} />
            <Select label="Shift Type" value={form.shiftType} onChange={(v: any) => f("shiftType", v)} options={[...SHIFT_TYPES]} />
            <Select label="Assigned Group" value={form.group} onChange={(v: any) => f("group", v)} options={GROUPS} />
            <Input label="Join Date" type="date" value={form.joinDate} onChange={(v: any) => f("joinDate", v)} />
            <Input label="Experience" value={form.experience} onChange={(v: any) => f("experience", v)} placeholder="e.g. 3 years" />
            <Input label="Monthly Salary ($)" type="number" value={String(form.salary)} onChange={(v: any) => f("salary", Number(v))} />
            <Select label="Status" value={form.status} onChange={(v: any) => f("status", v)} options={["Active", "Inactive"]} />
            <Input label="Emergency Contact" value={form.emergencyContact} onChange={(v: any) => f("emergencyContact", v)} />
            <Input label="Emergency Phone" value={form.emergencyPhone} onChange={(v: any) => f("emergencyPhone", v)} />
            <div className="col-span-2">
              <Input label="Address" value={form.address} onChange={(v: any) => f("address", v)} />
            </div>
            <div className="col-span-2">
      ...
      {modal === "duty" && selected && (
        <Modal title={`Assign Duty — ${selected.name}`} onClose={() => setModal(null)}>
          <div className="space-y-4">
            ...
            <Input label="Duty Date" type="date" value={dutyForm.date} onChange={(v: any) => setDutyForm(p => ({ ...p, date: v }))} />
            <Textarea label="Notes" value={dutyForm.notes} onChange={(v: any) => setDutyForm(p => ({ ...p, notes: v }))} placeholder="Any special instructions..." rows={2} />
          </div>
