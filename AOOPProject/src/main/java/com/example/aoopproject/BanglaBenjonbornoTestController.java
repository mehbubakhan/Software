...
public class BanglaBenjonbornoTestController {
    ...
    /* ===== Data ===== */
    // Full set incl. retroflex & signs often taught with consonants
    ...
    // 👇 matches your screenshot folder
    ...
    private final int TOTAL_QUESTIONS = Math.min(15, BYANJON.size());
    ...
    public void initialize() {
        ...
        titleLabel.setText("বাংলা বর্ণ পরীক্ষা — ব্যঞ্জনবর্ণ");
    ...
    private void handleExamStart(ActionEvent e) {
        ...
        loadCurrent();   // build 4 options, enable them
    ...
    private void loadCurrent() {
        ...
        // Build 4 options (1 correct + 3 unique distractors)
        LinkedHashSet<String> opts = new LinkedHashSet<>();
        opts.add(target);
        while (opts.size() < 4) {
            String s = BYANJON.get(rng.nextInt(BYANJON.size()));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            String v = list.get(i);
            b.setText(v);          // show the letter on the button
            b.setDisable(false);
            b.setStyle("");
    ...
    private List<String> pickQuestions(int n) {
        List<String> pool = new ArrayList<>(BYANJON);
