...
/**
 * English Level 1:
 * - Mixed representation per option: EN digit OR EN word (0..100)
 * - Voice says the target number in digits.
 * - 15 questions, no timer.
 */
public class MathTestLevel1Controller {
    ...
    private enum Render { EN_DIGIT, EN_WORD }
    ...
    public void initialize() {
        ...
        titleLabel.setText("Math Test — Level 1 (English)");
        questionLabel.setText("Press Start, listen to the voice, and select the correct answer (number/word).");
    ...
    private void handleExamStart(ActionEvent e) {
        ...
        speakCurrent();     // speak once per question
        updateProgress();
    }
    ...
    private void loadQuestion() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // Mix representation
            Render r = rng.nextBoolean() ? Render.EN_DIGIT : Render.EN_WORD;
            b.setText(r == Render.EN_DIGIT ? String.valueOf(val) : capitalize(EN[val]));
        ...
        answered = false;
        nextButton.setDisable(true);
        questionLabel.setText("Listen and pick the matching number or its word.");
    }
