...
interface Payment {
  ...
  parentName: string;
  amount: number;
  paymentType: 'Application Fee' | 'Processing Fee' | 'Documentation Fee' | 'Home Study Fee' | 'Legal Fee' | 'Donation';
...
export default function PaymentManagement() {
  ...
  const handleVerifyPayment = (paymentId: string) => {
    ...
    setPayments(payments.map(p =>
      p.id === paymentId ? { ...p, status: 'Completed' as const, receiptNumber: `RCP-${Date.now()}` } : p
    ));
