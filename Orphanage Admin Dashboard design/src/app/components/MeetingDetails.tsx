...
export default function MeetingDetails({ meeting, onClose, onUpdate, onCancel }: MeetingDetailsProps) {
  ...
  const calculateCompatibilityScore = () => {
    const parentAvg = Object.values(parentEvaluation).reduce((a: number, b: number) => a + b, 0) / 4;
    const childAvg = Object.values(childObservation).reduce((a: number, b: number) => a + b, 0) / 4;
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-gray-200 rounded-full h-4">
                    <div
                      ...
                      style={{ width: `${Object.values(parentEvaluation).reduce((a: number, b: number) => a + b, 0) / 4}%` }}
                    ></div>
                  </div>
                  <span className="text-2xl font-bold text-gray-900">
                    {Math.round(Object.values(parentEvaluation).reduce((a: number, b: number) => a + b, 0) / 4)}
                  </span>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-gray-200 rounded-full h-4">
                    <div
                      ...
                      style={{ width: `${Object.values(childObservation).reduce((a: number, b: number) => a + b, 0) / 4}%` }}
                    ></div>
                  </div>
                  <span className="text-2xl font-bold text-gray-900">
                    {Math.round(Object.values(childObservation).reduce((a: number, b: number) => a + b, 0) / 4)}
                  </span>
                </div>
              </div>
            </div>
